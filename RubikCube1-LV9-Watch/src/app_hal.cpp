...
struct HourlyForecast {
  ...
  int icon; // (0-7) // 0 - sun + cloud // 1 - sun // 2 - snow // 3 - rain // 4
            // - clouds // 5 - tornado // 6 - wind // 7 - sun + haze
  int temp; //
  int uv;   // uv index
  int humidity; // %
...
void onVolumeDown(lv_event_t *e) { setNoContacts(); }
...
int getPrefInt(const char *key, int def_value) { return def_value; }
...
bool loadCustomFace(const char *file) { return true; }
...
void setupWeather() {
  // lv_obj_set_style_bg_img_src(ui_weatherScreen, &ui_img_753022056,
  ...
  for (int h = 0; h < 24; h++) {
    addHourlyWeather(hourly[h].hour, hourly[h].icon, hourly[h].temp,
                     hourly[h].humidity, hourly[h].wind, hourly[h].uv, false);
  }
}
...
void update_faces() {
  ...
  ui_update_watchfaces(second, minute, hour, mode, am, day, month, year,
                       weekday, temp, icon, battery, connection, steps,
                       distance, kcal, bpm, oxygen);
}
